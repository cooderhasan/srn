...
async function parseDumpFile() {
    ...
    console.log("Searching for category IDs with 'gazelle' in name...");
    ...
    for await (const line of rl) {
        ...
        if (currentTable === 'ps_category_lang' && trimmed.startsWith('(')) {
            ...
            for (const tuple of tuples) {
                ...
                // Schema: id_category, id_shop, id_lang, name, ...
                const id = vals[0];
                const name = String(vals[3]).toLowerCase();

                if (name.includes('samara')) {
                    console.log(`Found Category: ID=${id}, Name="${vals[3]}"`);
                }
            }
        }
    }
