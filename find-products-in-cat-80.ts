...
async function parseDumpFile() {
    ...
    for await (const line of rl) {
        ...
        if (currentTable === 'ps_category_product' && trimmed.startsWith('(')) {
            ...
            for (const tuple of tuples) {
                ...
                // Schema: id_category, id_product, position (usually)
        ...
        if (currentTable === 'ps_product' && trimmed.startsWith('(')) {
            ...
            for (const tuple of tuples) {
                const vals = parseTuple(tuple);
                // Schema: id_product (0), ..., id_category_default (3 usually)
        ...
        if (currentTable === 'ps_product_lang' && trimmed.startsWith('(')) {
            ...
            for (const tuple of tuples) {
                const vals = parseTuple(tuple);
                // Schema: id_product (0), id_shop (1), id_lang (2), ..., name (9)
                const id = Number(vals[0]);
                const langId = Number(vals[2]);

                if (targetProductIds.has(id)) {
                    // Try to find name at index 9
                    const name = vals[9];
                    // Only capture if lang is 1 (or 2 if 1 missing, but just take all for now)
    ...
    console.log(`Found ${foundProducts.length} products associated with category ${targetCategoryId} (Default or Secondary):`);
    foundProducts.forEach(p => console.log(`- ${p.name} (ID: ${p.id})`));
}
